...
using MGroup.MSolve.Logging.Postprocessing;
...
namespace MGroup.MSolve.Logging.VTK
{
    ...
    public class VtkLog2D : IAnalyzerLog
    {
        ...
        public VtkLog2D(string directory, string filename, IModel model, VtkMesh<INode> vtkMesh, 
            ...
            using (var writer = new VtkFileWriter(path))
            {
                ...
                if (logStrains || logStresses || (vonMisesStressCalculator != null))
                {
                    ...
                    if (vonMisesStressCalculator != null)
                    {
                        ...
                        for (int i = 0; i < numPoints; ++i)
                        {
                            INode node = vtkMesh.OriginalNodes[i];
